...
class KeysightE5080B(Instrument):
    ...
    def check_errors(self):
        ...
        if not err.strip().startswith("0"):
            print(f"Instrument Error: {err.strip()}")
...
try:
    ...
    vna.preset()
    time.sleep(5)  # allow preset to complete
    ...
    try:
        ...
        print(f"Warning: calibration load skipped ({e}).")
    ...
    def set_magnet_field(field):
        ...
        time.sleep(1)  # simulate magnet ramp
    ...
    for field, data in results.items():
        mag_db = 20 * np.log10(np.abs(data) + 1e-12)  # avoid log(0)
    ...
    with open(OUTPUT_CSV, mode="w", newline="") as f:
        ...
        writer.writerow([f"# Frequency range: {START_FREQ/1e9:.3f}-{STOP_FREQ/1e9:.3f} GHz"])
        ...
        for i, freq in enumerate(freqs):
            ...
            for field in FIELDS:
                trace = results[field]
                row.append(20 * np.log10(np.abs(trace[i]) + 1e-12))
            writer.writerow(row)
